...
shape2 <- shape %>%
  mutate(x = x + 1,
         group = 2) %>%
  mutate(x = (x - (midpoint_x+1))*cos(rotation)-(y-midpoint_y)*sin(rotation) + (midpoint_x+1),
         y = (x - (midpoint_x+1))*sin(rotation)-(y-midpoint_y)*cos(rotation) + (midpoint_y+1))
...
shape_full <- bind_rows(shape,shape2)


data <- shapes(5000,.2,0,10,0,10,1) %>%
  group_by(group) %>%
  mutate(fill = sample(colorRampPalette(c("#116530","#14B1AB","#52006A","#CD113B","#FF7600","#FFA900"))(200))) %>%
  rowwise() %>%
